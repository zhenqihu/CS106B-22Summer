...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) { 
    ...
    for (string state: populations.keys()) {
        seats.put(state, 1);
        numSeats--;
        DataPoint stateWeight = {state, - populations[state] / sqrt(2)};
        queue.enqueue(stateWeight);
    }
    while (numSeats > 0) {
        DataPoint assigned = queue.dequeue();
        string state = assigned.name;
        seats[state]++;
        numSeats--;
        int seat = seats[state];
        DataPoint newState = {state, - populations[state] / sqrt(seat * (seat + 1))};
