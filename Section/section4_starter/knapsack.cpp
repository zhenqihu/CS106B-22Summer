...
int fillBackpackHelper(Vector<BackpackItem>& items, int capacityRemaining, int curValue, int index) {
    if (capacityRemaining < 0) {
        ...
        int valueWith = fillBackpackHelper(items, capacityRemaining - item.weight, curValue + item.survivalValue, index + 1);
        int valueWithout = fillBackpackHelper(items, capacityRemaining, curValue, index + 1);
