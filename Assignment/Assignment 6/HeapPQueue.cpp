...
DataPoint HeapPQueue::dequeue() {
    ...
    logicalSize--;
    int index = 1;
    while (index <= logicalSize / 2) {
        int left = 2 * index;
        int right = 2* index + 1;
        if (right <= logicalSize && elems[right].weight < elems[left].weight) {
            if (elems[index].weight > elems[right].weight) {
                swap(elems[index], elems[right]);
                index = right;
            } else {
                break;
            }
        } else if (elems[index].weight > elems[left].weight) {
            swap(elems[index], elems[left]);
            index = left;
        } else {
            break;
        }
...
void HeapPQueue::doubleSize() {
    DataPoint* elemsDouble = new DataPoint[2 * allocatedSize];
    for (int i = 1; i <= logicalSize; i++) {
        elemsDouble[i] = elems[i];
    }
