...
bool allLess(TreeNode* node, int target) {
    if (node == nullptr) {
        return true;
    } else {
        return node->data <= target && allLess(node->right, target);
    }
}

bool allBigger(TreeNode* node, int target) {
    if (node == nullptr) {
        return true;
    } else {
        return node->data >= target && allBigger(node->left, target);
    }
}
...
bool isBST(TreeNode* root) {
    if (root == nullptr) {
        ...
        return isBST(root->left) && isBST(root->right)
               && allLess(root->left, root->data) && allBigger(root->right, root->data);
    }
}
