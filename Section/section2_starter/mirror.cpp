...
void mirror(Grid<int> &nums){
    for (int row = 0; row < nums.numRows(); row++) {
        for (int col = row + 1; col < nums.numCols(); col++) {
            int temp = nums.get(row, col);
            nums.set(row, col, nums.get(col, row));
            nums.set(col, row, temp);
        }
    }
}
